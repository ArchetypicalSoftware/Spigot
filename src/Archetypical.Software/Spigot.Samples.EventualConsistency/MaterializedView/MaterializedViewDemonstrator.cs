...
{
    public class MaterializedViewDemonstrator : IDemonstrator
    {
        private readonly MessageSender<Record> _sender;
        private readonly RecordKnob _recordKnob;
        ...
        public MaterializedViewDemonstrator(MessageSender<Record> sender, RecordKnob recordKnob)
        {
            _sender = sender;
            _recordKnob = recordKnob;
        }
        ...
        {
            _recordKnob.Open += (a) => writer.WriteLine($"{a.EventData.Time.ToShortDateString()} - Terminal {a.EventData.Terminal} Gate {a.EventData.Gate} : {a.EventData.Airline} flight number {a.EventData.FlightNumber} {(a.EventData.Transaction == "ARR" ? "Arrived" : "Departed")}");

            using (var str =
                new WebClient().OpenRead("https://data.sfgov.org/api/views/chfu-j7tc/rows.csv?accessType=DOWNLOAD"))
            {
                await ProcessStreamAsync(str);
            }
                ...
                do
                {
                    ...
                    if (position != null)
                    {
                        ...
                        if (!isHeader)
                        {
                            ...
                            var gate = terminal.Gates.GetOrAdd(record.Gate, new Gate(record.Gate, _sender, source));
